...
class MusicBot(discord.Client):
    ...
    def owner_only(func):
        ...
        async def wrapper(self, *args, **kwargs):
            ...
            if not orig_msg or orig_msg.author.id == self.config.owner_id:
                ...
                raise exceptions.PermissionsError("only the owner can use this command silly :P", expire_in=30)
        ...
        for channel in channels:
            ...
            if channel and channel.type == discord.ChannelType.voice:
                ...
                if not chperms.connect:
                    self.safe_print("Can't join channel \"%s\", no permission." % channel.name)
                    continue

                elif not chperms.speak:
                    self.safe_print("I won't join channel \"%s\", because i have no permission to speak." % channel.name)
                    continue
        ...
        if server.id not in self.players:
            if not create:
                raise exceptions.CommandError(
                    ...
                    'Use %sjoin to make it join ye voice channel.' % self.config.command_prefix)
    ...
    async def cmd_join(self, channel, author, voice_channel):
        ...
        if not chperms.connect:
            self.safe_print("Cannot join channel \"%s\", because no permission." % author.voice_channel.name)
            return Response(
                "```Cannot join channel \"%s\", because no permission.```" % author.voice_channel.name,
                delete_after=25
            )

        elif not chperms.speak:
            self.safe_print("I won't join channel \"%s\", because no permission to speak." % author.voice_channel.name)
            return Response(
                "```I won't join channel \"%s\", because no permission to speak.```" % author.voice_channel.name,
                delete_after=25
            )
    ...
    async def cmd_extracmd_volume(self, message, player, new_volume=None):
        """
        Usage:
        ...
        if not new_volume:
            return Response('Current TTS volume: `%s%%`' % int(player.volume * 100), reply=True, delete_after=20)
    ...
    async def cmd_tts_perms(self, author, channel, server, permissions):
        ...
        return Response("Sent :mailbox_with_mail:", delete_after=20)
    ...
    async def cmd_enuk(self, player, leftover_args, msg):
        self.safe_print("[cmd_enuk]")
        msg = ' '.join([msg, *leftover_args])
        self.safe_print("[cmd_enuk] {}".format(msg))
        try:
            tts = gTTS(text=msg, lang='en-uk')

            tts.save("enuk.mp3")
            player.playFile(f="enuk.mp3")
            #f = TemporaryFile()
            #tts.write_to_fp(f)
            #f.close()
            #player.playFile(f=f)tts-scammer 
        except Exception as e:
            self.safe_print("[cmd_enuk] {}".format(e))

    async def cmd_mp3(self, player, leftover_args, msg):
        self.safe_print("[cmd_mp3]")
        msg = ' '.join([msg, *leftover_args])
        self.safe_print("[cmd_mp3] {}".format(msg))
        try:
            player.playFile(f="test.mp3")
            #f = TemporaryFile()
            #tts.write_to_fp(f)
            #f.close()
            #player.playFile(f=f)tts-scammer 
        except Exception as e:
            self.safe_print("[cmd_mp3] {}".format(e))


    async def cmd_wexican(self, player, leftover_args, msg):
        self.safe_print("[cmd_wexican]")
        msg = ' '.join([msg, *leftover_args])
        self.safe_print("[cmd_wexican] {}".format(msg))
        try:
            tts = gTTS(text=msg, lang='es-us')

            tts.save("es-us_wexican.mp3")
            player.playFile(f="es-us_wexican.mp3")
            #f = TemporaryFile()
            #tts.write_to_fp(f)
            #f.close()
            #player.playFile(f=f)tts-scammer 
        except Exception as e:
            self.safe_print("[cmd_wexican] {}".format(e))

    async def cmd_enau(self, player, leftover_args, msg):
        self.safe_print("[cmd_enau]")
        msg = ' '.join([msg, *leftover_args])
        self.safe_print("[cmd_enau] {}".format(msg))
        try:
            tts = gTTS(text=msg, lang='en-au')

            tts.save("enau.mp3")
            player.playFile(f="enau.mp3")
            #f = TemporaryFile()
            #tts.write_to_fp(f)
            #f.close()
            #player.playFile(f=f)
        except Exception as e:
            self.safe_print("[cmd_enau] {}".format(e))

    async def cmd_scammer(self, player, leftover_args, msg):
        self.safe_print("[cmd_scammer]")
        msg = ' '.join([msg, *leftover_args])
        self.safe_print("[cmd_scammer] {}".format(msg))
        try:
            tts = gTTS(text=msg, lang='hi')

            tts.save("scammer.mp3")
            player.playFile(f="scammer.mp3")
            #f = TemporaryFile()
            #tts.write_to_fp(f)
            #f.close()
            #player.playFile(f=f)
        except Exception as e:
            self.safe_print("[cmd_scammer] {}".format(e))

    async def cmd_ru(self, player, leftover_args, msg):
        self.safe_print("[cmd_ru]")
        msg = ' '.join([msg, *leftover_args])
        self.safe_print("[cmd_ru] {}".format(msg))
        try:
            tts = gTTS(text=msg, lang='ru')

            tts.save("ru.mp3")
            player.playFile(f="ru.mp3")
            #f = TemporaryFile()
            #tts.write_to_fp(f)
            #f.close()
            #player.playFile(f=f)
        except Exception as e:
            self.safe_print("[cmd_ru] {}".format(e))

    async def cmd_enus(self, player, leftover_args, msg):
        msg = ' '.join([msg, *leftover_args])
        try:
    ...
    @owner_only
    async def cmd_extracmd_setname(self, leftover_args, name):
        """
        Usage:
        ...
        return Response("done :ok_hand:", delete_after=20)

    @owner_only
    async def cmd_extracmd_setnick(self, server, channel, leftover_args, nick):
        """
        Usage:
    ...
    @owner_only
    async def cmd_extracmd_setavatar(self, message, url=None):
        """
        Usage:
    ...
    async def cmd_help(self, message):
        em = discord.Embed(title="Help", url='http://codfishyfishy.co.uk/talkingmadarchod.html', description="Codfish246#1642's Voice channel TTS bot, Current supports enus, enuk, scammer (hindi), enau, ru (russian), tw (chinese/taiwanese), kr (korean) and jp (japanese)", color=0x00ff00)
        em.set_author(name="Talking Madarchod", url='http://codfishyfishy.co.uk/talkingmadarchod.html', icon_url='https://i.ytimg.com/vi/KCYR9418I98/maxresdefault.jpg')
        em.set_thumbnail(url='https://i.ytimg.com/vi/KCYR9418I98/maxresdefault.jpg')
        return Response(embed=em)
        #return Response(embed=embed)
        #await self.bot.say(embed=embed)
    """

    async def cmd_extracmd_disconnect(self, server):
        await self.disconnect_voice_client(server)
        return Response("ok bai", delete_after=20)

    async def cmd_extracmd_restart(self, channel):
        await self.safe_send_message(channel, "restarting :wave:")
        await self.disconnect_all_voice_clients()
        raise exceptions.RestartSignal

    async def cmd_extracmd_shutdown(self, channel):
        await self.safe_send_message(channel, "bai bai :wave:")
        await self.disconnect_all_voice_clients()
